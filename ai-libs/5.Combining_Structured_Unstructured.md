...
4. Ensure PyKX is installed:
    ```bash
    pip install pykx~=3.2
    ```
...
Next you will need to make sure [PyKX Under Q](https://code.kx.com/pykx/pykx-under-q/intro.html#prerequisites) is installed, you will likely need to run the below after lauching q:
...
Now, via PyKX we can easily import PyTorch, load the sentence transformer class and import the dynamic quantization utility. For optimal parallelisation we can set our PyTorch threads to number of secondary threads minus one (e.g. `4-1=3` in our case).
...
Relevant PyKX concepts used in the above include:
